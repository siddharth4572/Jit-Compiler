...
| IF LPAREN expr RPAREN stmt %prec LOWER_THAN_ELSE {
    ...
    auto* block = dynamic_cast<BlockStmt*>($5);
    if (block) {
        thenStmts = std::move(block->stmts);
        delete block;
    } else if ($5) {
        thenStmts.emplace_back(std::unique_ptr<Stmt>($5));
    }
    $$ = new IfStmt(std::unique_ptr<Expr>($3), std::move(thenStmts));
}
| IF LPAREN expr RPAREN stmt ELSE stmt {
    std::vector<std::unique_ptr<Stmt>> thenStmts, elseStmts;

    auto* blockThen = dynamic_cast<BlockStmt*>($5);
    if (blockThen) {
    ...
    auto* blockElse = dynamic_cast<BlockStmt*>($7);
    if (blockElse) {
        elseStmts = std::move(blockElse->stmts);
        delete blockElse;
    } else if ($7) {
        elseStmts.emplace_back(std::unique_ptr<Stmt>($7));
    }

    $$ = new IfStmt(std::unique_ptr<Expr>($3),
                    std::move(thenStmts),
                    std::move(elseStmts));
}
